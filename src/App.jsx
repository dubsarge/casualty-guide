...
const WEAPONS = [
  ...
  {id:"knife",cat:"Knife / Blade",guns:["Hunting Knife","Bowie Knife","Machete","Cleaver","Hatchet","Broken Bottle","Bayonet","Sword"],sevMod:0,
    ...
    doHint:"blade"},
  {id:"hammer",cat:"Blunt / Crush Weapon",guns:["Hammer","Pickaxe","Rock/Stone","Rifle Butt","Boot (Stomping)","Horse Kick","Wagon Wheel","Falling Object"],sevMod:0,
...
const needsWeapon = (type) => isGSW(type) || isStabOrArrow(type) || isCrush(type);
...
const OD = {
  ...
  gsw_graze:{sym:{1:["Bullet skimmed the surface — long shallow furrow, lots of blood but no penetration","Burns along the graze from bullet heat"],2:["Deep graze carving a channel through skin and muscle","Heavy bleeding, raw and exposed","Possible bone chip if over bone"]},
    ...
    doEx:{1:["/do A long furrow across their {z} where the bullet skimmed. Bleeding and raw, but didn't go in."],2:["/do Deep graze carving a channel through {z}. Muscle visible in the furrow. Bleeding heavy."]},
    wDoEx:{handgun:{1:["/do Revolver round skimmed across {z}, leaving a shallow burn-like furrow. Bleeding but intact."],2:["/do Pistol round carved a deep channel across {z}. Muscle exposed in the furrow. Powder burns blackening the edges."]},sawnoff:{1:["/do Shotgun pellets peppered across {z}. Dozens of tiny holes, bleeding from each. Skin stippled and raw."],2:["/do Sawed-off blast raked across {z}. Shallow but wide — pellets scattered across the surface, each one bleeding."]},repeater:{1:["/do Repeater round grazed {z}, tearing a deeper furrow than a pistol would leave. Raw, bleeding steadily."],2:["/do Repeater round carved a deep channel through {z}. Higher velocity means the graze goes deeper than it looks."]},shotgun:{1:["/do Shotgun pellets grazed {z}. Scattered pattern of shallow wounds across the area, each one stinging and bleeding."],2:["/do Shotgun blast caught the edge of {z}. Wide swath of pellet marks — some shallow, some dug in. Bleeding from a dozen points."]},rifle:{1:["/do High-caliber round grazed {z}. Even the graze carved a deep furrow — the energy bruised tissue around the channel."],2:["/do Rifle round skimmed {z}, tearing a wide, deep channel. Surrounding tissue is bruised from the shockwave alone."]}},
  ...
  fracture_comminuted:{sym:{4:["Bone shattered — limb unsalvageable"],5:["Catastrophic limb destruction, shock"]},signs:["🦴 Destruction: Bone in multiple pieces — structurally gone","🩸 Bleeding: Massive — fragments shred vessels from inside","😰 Shock: Immediate — pale, cold, rapid thready pulse","⚡ No sensation: Complete loss below injury","💀 Tissue death: Color changing, limb going cold — blood supply destroyed"],doEx:{4:["/do Their {z} destroyed. Bone fragments through flesh. No feeling below. Limb cold. Shock."],5:["/do Their {z} is obliterated — bone in a hundred pieces, each one shredding vessels and muscle from the inside. Limb is dead weight. Grey, cold, gone. Shock is killing them."]},rec:["Losing a limb — permanent","Extended recovery, daily bandages","Phantom pain to explore","Character-defining moment"],drugs:["Ether — must be unconscious","Morphine post-op — highly addictive","Prolonged exposure — addiction mechanic kicks in"]},
  ...
  amputation:{sym:{4:["Limb or digit severed or so destroyed it must be removed","Catastrophic bleeding from the stump — bright arterial spurts","Immediate shock — pale, cold, clammy, pulse racing then fading","The severed part may still be twitching — nerves firing independently"],5:["Complete traumatic loss — limb torn away by force","Massive hemorrhage, tissue hanging, bone exposed","Consciousness fading rapidly — minutes without a tourniquet"]},
    ...
    doEx:{4:["/do Their {z} is gone — severed above the joint. Bone visible in the ragged stump. Blood spurting in rhythmic pulses. Skin white as parchment. Shaking violently."],5:["/do What's left of their {z} is a ruin of torn muscle and shattered bone. Blood everywhere. Eyes glassy, barely conscious. The severed piece lies nearby."]},
...
function CombinedOut({injuries,effSev}){
  ...
  return React.createElement("div",{style:{background:"rgba(139,34,34,.04)",border:"2px solid rgba(178,34,34,.2)",borderRadius:10,padding:"12px 13px",marginTop:12,marginBottom:8}},
    React.createElement("h3",{style:{fontFamily:"'Playfair Display',Georgia,serif",fontSize:14,margin:"0 0 8px",color:"#8b2222"}},"\u26A0 Combined Injury Assessment"),
    ...
    React.createElement(P,{style:{fontSize:10.5,fontStyle:"italic",color:"#8b7355",marginTop:6}},"These interactions are suggestions for layering depth \u2014 use what resonates with your character.")
  );
}
  ...
  return(<div style={{minHeight:"100vh",background:"#faf3e8",fontFamily:"Georgia,serif",color:"#3d2b1f"}}>
    <div style={{background:"linear-gradient(180deg,#3d2b1f,#5c4033)",padding:"18px 12px 14px",textAlign:"center",borderBottom:"4px solid #c4a882",position:"relative"}}>
      ...
      <div style={{background:"rgba(244,232,209,.12)",border:"1px solid rgba(196,168,130,.3)",borderRadius:8,padding:"10px 16px",margin:"10px auto 6px",maxWidth:520}}>
        <p style={{color:"#f4e8d1",fontSize:13,margin:"0 0 4px",fontWeight:"bold",fontFamily:"Georgia,serif"}}>⚕ This is your RP starting point — not a script.</p>
        <p style={{color:"#c4a882",fontSize:11,margin:0,lineHeight:1.5,fontStyle:"italic"}}>Click where you're hurt. See what's underneath. Get symptoms to describe, /do examples to build on, and recovery guidance. Make it yours — your character, your voice, your story.</p>
      </div>
      <p style={{color:"#8b7355",fontSize:8.5,margin:"6px 0 0",fontStyle:"italic"}}>Created by Sr. Dr. Thomas 'Tommy' Denholme — MMO — Chief of Neurology, M.D., Ph.D., DMSc., FRCS</p>
    </div>
    ...
    <div style={{display:"flex",background:"#efe3d0",borderBottom:"2px solid #c4a882"}}>
      ...
      <button onClick={function(){window.open("https://forms.gle/wMmBdaa6wKjFpo49A","_blank");}} style={{flex:1,padding:"8px 0",background:"transparent",border:"none",borderBottom:"3px solid transparent",color:"#b22222",fontFamily:"Georgia,serif",fontSize:12.5,fontWeight:"bold",cursor:"pointer"}}>📝 Issues / Improvements</button>
    </div>

    <div style={{padding:"12px 10px 80px"}}>
      {tab==="injuries"?(<div className="cg-layout">
        <div className="cg-paperdoll">
            <div style={{display:"flex",gap:3,marginBottom:5}}>{[["Front",false],["Back",true]].map(([l,v])=><button key={l} onClick={()=>setBk(v)} style={{padding:"4px 14px",borderRadius:10,border:"1px solid #c4a882",background:bk===v?"#3d2b1f":"transparent",color:bk===v?"#f4e8d1":"#8b7355",fontSize:11,fontWeight:"bold",cursor:"pointer",fontFamily:"Georgia,serif"}}>{l}</button>)}</div>
        ...
        <div className="cg-injuries">
            ...
            {injs.map((inj,i)=>{const zone=Z.find(z=>z.id===inj.zid);const avail=zone?gzi(zone):[];const isA=act===i;
              return(<div key={i} onClick={()=>setAct(i)} style={{background:isA?"rgba(205,133,63,.08)":"rgba(61,43,31,.02)",border:`2px solid ${isA?"#cd853f":"#ddd0bc"}`,borderRadius:9,padding:"9px 10px",cursor:"pointer",transition:"all .15s"}}>
                ...
                {zone&&<select value={inj.type||""} onChange={e=>{setInjs(p=>p.map((x,j)=>j===i?{...x,type:e.target.value,weapon:"",intDmg:"",intSev:0}:x));}} onClick={e=>e.stopPropagation()} style={sel}><option value="">— What happened here? —</option>{avail.map(k=><option key={k} value={k}>{IT[k]&&IT[k].i} {IT[k]&&IT[k].l}</option>)}</select>}
                {inj.type&&needsWeapon(inj.type)&&(
                  <select value={inj.weapon||""} onChange={e=>{setInjs(p=>p.map((x,j)=>j===i?{...x,weapon:e.target.value}:x));}} onClick={e=>e.stopPropagation()} style={{...sel,marginTop:4,borderColor:inj.weapon?"#6b8f3c":"#c4a882"}}>
                    ...
                    {(isStabOrArrow(inj.type)?WEAPONS.filter(w=>w.id==="bow"||w.id==="knife"):isCrush(inj.type)?WEAPONS.filter(w=>w.id==="hammer"):WEAPONS.filter(w=>w.id!=="knife"&&w.id!=="hammer")).map(w=>(
                ...
                {inj.intDmg&&(
                  <div style={{display:"flex",gap:2,marginTop:4}}>{INT_SEV.map(s=>
                    <button key={s.v} onClick={e=>{e.stopPropagation();setInjs(p=>p.map((x,j)=>j===i?{...x,intSev:s.v}:x));}} style={{flex:1,padding:"4px 0",border:"2px solid "+(inj.intSev===s.v?s.c:"transparent"),borderRadius:5,background:inj.intSev===s.v?s.c:"rgba(61,43,31,.05)",color:inj.intSev===s.v?"#fff":"#8b7355",cursor:"pointer",fontSize:9,fontWeight:"bold",fontFamily:"Georgia,serif"}}>{s.l}</button>
                  )}</div>
                )}
      ...
      {has&&tab==="injuries"&&(<div style={{background:"#fff9f0",border:"2px solid #c4a882",borderRadius:12,padding:"14px 11px",boxShadow:"0 2px 8px rgba(61,43,31,.06)"}}>
        ...
        {val.map((inj,i)=>{const zone=Z.find(z=>z.id===inj.zid);const data=OD[inj.type];if(!data||!zone)return null;
          ...
          return(<div key={i} style={{marginBottom:10,border:"1px solid #c4a882",borderRadius:10,overflow:"hidden"}}>
            <div style={{background:"rgba(205,133,63,.08)",padding:"8px 11px",borderBottom:"1px solid #c4a882",position:"sticky",top:0,zIndex:1}}>
              <div style={{display:"flex",alignItems:"center",gap:5,flexWrap:"wrap"}}><span style={{fontSize:15}}>{IT[inj.type]&&IT[inj.type].i}</span><span style={{fontFamily:"'Playfair Display',Georgia,serif",fontSize:14,fontWeight:"bold"}}>{IT[inj.type]&&IT[inj.type].l}</span>{wm&&<span style={{fontSize:11,color:"#556b2f",fontWeight:"bold"}}>({getGunName(inj.weapon)||wm.cat})</span>}{idmg&&<span style={{fontSize:11,color:"#8b2222",fontWeight:"bold"}}>+ {idmg.l}</span>}<span style={{fontSize:10.5,color:"#8b7355"}}>— {zone.label}</span><span style={{background:si.c,color:"#fff",padding:"1px 6px",borderRadius:10,fontSize:9,fontWeight:"bold"}}>{si.l}</span></div>
            </div>
            <div style={{maxHeight:400,overflowY:"auto",padding:"10px 11px"}}>
            ...
            {idmg&&iSev>0&&<div style={{background:"rgba(139,34,34,.04)",borderRadius:5,padding:"6px 9px",marginBottom:7,border:"1px solid rgba(139,34,34,.15)"}}>
              <P style={{fontSize:10.5,fontWeight:"bold",color:"#8b2222"}}>🩻 {idmg.l} — {INT_SEV[iSev-1].l}</P>
              {intEffects.map((e,j)=><P key={j} style={{fontSize:11}}>• {e}</P>)}
            </div>}
            ...
            <Sec title="Recovery & Follow-Up" color="#cd6600"><div style={{display:"flex",gap:10,marginBottom:4,flexWrap:"wrap"}}><P style={{fontWeight:"bold",color:"#cd6600"}}>IC: {si.ic}</P><P style={{fontWeight:"bold",color:"#000",textDecoration:"underline"}}>OOC: {si.ooc}</P></div><P style={{fontSize:13,fontWeight:"bold",fontStyle:"italic",color:"#3d2b1f",textDecoration:"underline",marginBottom:6}}>These are just suggestive, follow your Doctor's advice and roleplay how you want.</P><P>{si.d}</P>{data.rec&&data.rec.map((r,j)=><P key={j}>• {r}</P>)}</Sec>
            {sv>=4&&(function(){var cm=getComaType([inj]);return cm?React.createElement("div",{style:{background:"rgba(74,0,0,.06)",borderRadius:7,padding:"9px 11px",marginBottom:8,border:"1px solid rgba(74,0,0,.2)"}},
              React.createElement("h4",{style:{fontFamily:"'Playfair Display',Georgia,serif",fontSize:12.5,margin:"0 0 5px",color:"#4a0000"}},"💀 Possible Outcome: Comatose Recovery"),
              React.createElement(P,{style:{fontStyle:"italic",color:"#5c4033",fontSize:11,marginBottom:6}},"At this severity, survival may mean waking up days later — or not at all. If you want to explore a coma arc, here's what it looks like for this type of injury:"),
              React.createElement(P,{style:{fontWeight:"bold",color:"#4a0000",fontSize:11.5,marginBottom:3}},cm.title),
              React.createElement(P,{style:{fontSize:11,marginBottom:5}},cm.desc),
              React.createElement(Sec,{title:"Signs of the Coma",color:"#4a0000"},cm.signs.map(function(s,j){return React.createElement(P,{key:j,style:{fontSize:11}},"• "+s);})),
              React.createElement(Sec,{title:"Coma /do Examples",color:"#6b8f3c"},cm.doEx.map(function(d,j){return React.createElement(Code,{key:j},d);})),
              React.createElement(Sec,{title:"Waking Up & Recovery RP",color:"#cd6600"},cm.recovery.map(function(r,j){return React.createElement(P,{key:j,style:{fontSize:11}},"• "+r);})),
              React.createElement(P,{style:{fontStyle:"italic",fontSize:10,color:"#8b7355",marginTop:4}},"A coma arc is entirely optional — discuss with your doc and anyone involved in the scene. It can create incredible long-form RP if everyone's on board.")
            ):null;})()}
    ...
    <div style={{background:"#3d2b1f",padding:"12px 16px",textAlign:"center",borderTop:"2px solid #c4a882"}}>
      <p style={{color:"#8b7355",fontSize:8.5,margin:0,fontFamily:"Georgia,serif",fontStyle:"italic"}}>Created by Sr. Dr. Thomas 'Tommy' Denholme — MMO — Chief of Neurology, M.D., Ph.D., DMSc., FRCS</p>
      <p style={{color:"#6b5b4a",fontSize:7.5,margin:"3px 0 0"}}>State of Monroe Medical Office — Ranch Roleplay — www.ranchroleplay.com</p>
    </div>
  </div>);
}
