      ...
      {has&&tab==="injuries"&&(<div style={{background:"#fff9f0",border:"2px solid #c4a882",borderRadius:12,padding:"14px 11px",boxShadow:"0 2px 8px rgba(61,43,31,.06)"}}>
        ...
        {val.map((inj,i)=>{const zone=Z.find(z=>z.id===inj.zid);const data=OD[inj.type];if(!data||!zone)return null;
          ...
          return(<div key={i} style={{marginBottom:10,border:"1px solid #c4a882",borderRadius:10,overflow:"hidden"}}>
            ...
            {(function(){var lw=getLethalityWarning(inj.zid,inj.type,inj.weapon,sv);return lw?React.createElement("div",{style:{background:lw.level==="fatal"?"rgba(74,0,0,.12)":"rgba(178,34,34,.08)",borderRadius:7,padding:"10px 12px",marginBottom:8,border:"2px solid "+(lw.level==="fatal"?"#4a0000":"#b22222")}},
              React.createElement("p",{style:{fontSize:13,fontWeight:"bold",color:lw.level==="fatal"?"#4a0000":"#b22222",margin:"0 0 4px",fontFamily:"Georgia,serif"}},"⚠️ "+(lw.level==="fatal"?"THIS INJURY PATTERN IS INCREDIBLY SEVERE AND WOULD MOST LIKELY RESULT IN DEATH":"⚠️ THIS INJURY PATTERN IS EXTREMELY DANGEROUS — SURVIVAL IS UNCERTAIN")),
              React.createElement(P,{style:{fontSize:11,color:"#5c4033",marginBottom:6}},lw.msg),
              React.createElement(P,{style:{fontSize:10.5,fontStyle:"italic",color:"#8b7355"}},"This is just a suggestion — you are encouraged to create your own story. This guide is a tool, not a rulebook. If you want your character to survive, work with your doc to make it happen. The best RP comes from the story you build together.")
            ):null;})()}
