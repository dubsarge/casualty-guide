...
function Body({back,markers,act,onClick}){
  ...
  return(
    <svg ref={ref} viewBox="0 0 300 510" onClick={hnd} style={{width:"100%",maxWidth:340,height:"auto",cursor:"crosshair",userSelect:"none"}}>
      <defs>
      ...
      {has&&tab==="injuries"&&(<div style={{background:"#fff9f0",border:"2px solid #c4a882",borderRadius:12,padding:"14px 11px",boxShadow:"0 2px 8px rgba(61,43,31,.06)"}}>
        ...
        {val.map((inj,i)=>{const zone=Z.find(z=>z.id===inj.zid);const data=OD[inj.type];if(!data||!zone)return null;
          ...
          return(<div key={i} style={{marginBottom:14,paddingBottom:14,borderBottom:"1px dashed #c4a882"}}>
            ...
            <Sec title="Recovery & Follow-Up" color="#cd6600"><div style={{display:"flex",gap:10,marginBottom:4,flexWrap:"wrap"}}><P style={{fontWeight:"bold",color:"#cd6600"}}>IC: {si.ic}</P><P style={{fontWeight:"bold",color:"#000",textDecoration:"underline"}}>OOC: {si.ooc}</P></div><P>{si.d}</P>{data.rec&&data.rec.map((r,j)=><P key={j}>• {r}</P>)}<P style={{fontSize:11.5,fontStyle:"italic",color:"#5c4033",textDecoration:"underline"}}>These are just suggestive, follow your Doctor's advice and roleplay how you want.</P></Sec>
